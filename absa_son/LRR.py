...
class LRR:
    ...
    def calcBeta(self):
        beta, retVal, flags = optimize.fmin_l_bfgs_b(func=self.maximumLikelihoodBeta, x0=self.beta,
                                                     fprime=self.gradBeta, args=(), m=5, maxiter=15000)
        converged = True
        if flags['warnflag'] != 0:
            converged = False
    ...
    def gradAlpha(self, x, *args):
        ...
        alphad = alphad.reshape((self.k, 1))
        rd, Sd, deltasq, mu, sigmaInv = args
        temp1 = np.single((np.dot(alphad.transpose(), Sd)[0][0] - rd)) * Sd
        temp1 /= deltasq
    ...
    def calcAlphaD(self, i):
        ...
        alphaD, retVal, flags = optimize.fmin_l_bfgs_b(func=self.maximumLikelihoodAlpha, x0=alphaD,
                                                       fprime=self.gradAlpha, args=Args, bounds=bounds, m=5,
                                                       maxiter=15000)
        converged = True
        if flags['warnflag'] != 0:
            converged = False
